...
export const GenericPage = ({ children }: { children: ReactChild }) => {
  ...
  const fetchCurrentMatch = async () => {
    ...
    try {
      ...
      console.log("this is current match -", data);
  ...
  const fetchMatchHistory = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        console.error(
          `❌ Failed to fetch match history. Status: ${res.status}`
        );
        if (res.status === 401) {
          console.warn("⚠️ Unauthorized request, removing token...");
  ...
  }, [state.isAuthenticated]); // ✅ Only run when authentication state changes
            ...
            >
              ...
              >
                ...
                <VStack align="stretch" flex={1} pt={isMobile ? "60px" : "0"}>
                  ...
                  {/* Current Match */}
                  {currentMatch && (
                    <ChatDialog match={currentMatch} isCurrent />
                  )}

                  {/* Match History - Filter Out Current Match */}
                  {matchHistory
                    .filter((match) => match.id !== currentMatch?.id) // Remove duplicates
                    .map((match) => (
